...
func CheckinListAllByDate(ctx *fiber.Ctx) error {
    ...
    q := database.Get()
    checkins, err := q.ListAllCheckinsByDate(
        context.Background(), sql.ListAllCheckinsByDateParams{
            StartDate: pgtype.Date{Time: *parsedStartDate, Valid: true},
            EndDate:   pgtype.Date{Time: *parsedEndDate, Valid: true},
        },
    )
    ...
    if len(checkins) == 0 {
        checkins = []sql.ListAllCheckinsByDateRow{}
    }

    return ctx.JSON(
        formatters.PagedResponse[sql.ListAllCheckinsByDateRow]{
            Count:    count,
            Next:     null.String{},
            Previous: null.String{},
            Results:  checkins,
